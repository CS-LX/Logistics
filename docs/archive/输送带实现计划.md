...
> 兼容：首发 `1.0.0.0` 前无公开旧档约束 — **无**迁移层；主仓 Factorio 带仅供参考、不打包，**不**作为并存约束  
...
| 取货点 | 与落点同一判据（`TryPeekItemFrom` / `RemoveItemFrom`）：正对端口只够得着那一端，侧向取格心。窗口半宽 `ComponentLogisticsHopper.BeltReach = 0.4`，**不再**是整格 ±0.5 —— 否则物品一进这格就被隔空取走，看着像在带中段凭空消失。该值须大于一个节拍内带走过的路程（默认 1 格/秒 × 0.25 秒），不然物品会在两次尝试之间冲过取货点 |
...
#### 弧长正向不变量（2026-07-26 修缺陷）

`Sign` 是**相对量**，只有先钉死「弧长正向」才有绝对含义。正向定义为：

> **弧长递增 = `BeltGeometry.Compare` 递增方向**（`OrderMembers` 从坐标序最小端排起，即 X→Y→Z 字典序小端）。平直带就是朝 +X / +Z。

`BeltPath.GetCellEnds` 靠邻居的 `SurfaceCenter` 把每格轨迹调转到这个方向；**单格组没有邻居可参照**，此前直接沿用 `GetRotationTrack` 的原始极性（rotation 0 → -Z、1 → -X），与多格正向相反，于是出现：单格带的滚动外观 / F2 `Sign` 与实际运物方向相反，接上第二格后又自行掉头。修法是给单格组补同一约定（平直带按水平坐标定向；坡道必有同轴邻居，走不到该分支）。

滚动 UV 本就按这条正向校准，故**不动视觉**；但铺设 `GetPlacementValue` 原先是按单格旧极性挑 `reverse` 的，须一并翻过来（视线朝正轴 → `reverse=0`），才能保证「落地朝玩家面前」且接长后不掉头。旧存档里已写入的 `reverse` 不受影响，多格带方向不变。

新增依赖带走向的逻辑（推实体、末端弹出、直角交接、料斗直连落点）一律经 `BeltPath`，**不要**自行从 `rotation` 推世界方向。
...
| `docs/archive/输送带实现计划.md` | 本文 | 已落地 / 验收记录 |
...
*发版收尾：2026-07-26 — 随 Logistics `1.0.0.0` 归档至 `docs/archive/`。*

...
> 状态：**施工中**（运物 P0 未开工；外观 / 自动布设已在库）  
> 范围：`Addons/Logistics` — 在现有外观 / 自动布设之上接入真实运物  
> 对照：Create Mechanical Belt、仓内 [Create传送带调研.md](archive/Create传送带调研.md)、SA `ComponentSAConveyerBelt`、物流 `AdvancedInserter` 的 `IInventory`  
...
对齐 Create：**一条直线（含坡）= 一个 Group**；物品仿真与库存只在 **Controller**；每格用 **IInventory 门面**给抓取机抽/塞「正在经过」的物品；位置是连续 `beltPosition`，不是每格真多槽。直角靠 **两个 Group 端点正交滑入** 并做速度继承。实体推动（SA 同款）与物品仿真解耦。
...
| 来源 | 用途 |
| --- | --- |
| [Create Wiki · Mechanical Belt](https://github.com/Creators-of-Create/Create/wiki/Mechanical-Belt) | 在途物非被动拾取；实体可被带；潜行可下带 |
| [Create传送带调研.md](archive/Create传送带调研.md) | Controller + `LinkedList` + `float beltPosition`；tick：推进 / 限速 / 末端；Segment 单槽门面 |
| SA `ComponentSAConveyerBelt` | 单格槽 + Progress；末端交付；站立推动 `Velocity`；通电连锁 |
| `ComponentAdvancedInserter` | `FindComponent<IInventory>()` 抽塞邻格 |
| 当前 Logistics `ConveyerBeltBlock` / `SubsystemConveyerBeltBlockBehavior` / `ConveyerBeltAnimatedTexture` | 几何 Data、自动朝向 / 爬坡（不转弯）、全局滚动贴图 |
...
| 实体 | 站上被带；潜行可下 | 同 SA 推 Velocity | 与物品仿真分离 |
...
1. 新增 `SubsystemBeltGroups`（或并入现有 Behavior 旁路子系统），`Logistics.xdb` 注册  
2. 邻接变更 / 放置 / 拆除时 Rebuild 连通分量  
3. 选举 Controller、维护有序 `Members`、存档 Guid 与成员  
4. 扩展现有 `SubsystemConveyerBeltBlockBehavior`：自动朝向 / 爬坡逻辑保留，Rebuild 时调用 Group 更新  
5. `Group.Sign` 默认 +1；暂无 UI（可用创造调试命令或临时改字段验证）

**验收**：连成一线（含坡）为同一 Group；打断后拆成两个；重进世界成员与 Controller 稳定。
...
| 路径 | 职责 | 现况 |
| --- | --- | --- |
| `Subsystems/SubsystemBeltGroups.cs` | Group 发现 / 存档 / tick 调度 | 待建 |
| `Belt/BeltGroup.cs` | Group 状态 | 待建 |
...
| `Subsystems/SubsystemConveyerBeltBlockBehavior.cs` | 放置、邻接自动朝向/爬坡 | ✅ 已有（后续接吸掉落与实体） |
| `Subsystems/SubsystemConveyerBeltVisuals.cs` | 滚动贴图驱动 | ✅ 已有 |
| `Drawing/ConveyerBeltAnimatedTexture.cs` | 滚动 RT | ✅ 已有；变速视觉后置 |
| `Logistics.xdb` | Subsystem + EntityTemplate | 部分已有 Visuals/Behavior；Group/Segment 待补 |
...
从 **P0 `SubsystemBeltGroups`** 开工：在现有自动朝向 / 爬坡 Behavior 上挂 Group 重建与存档，不先写运物。

*计划修订：2026-07-25 — 冻结 A/B；剔除 Factorio 并存与无实体兼容。*

...
> 状态：**施工中**（P0–P6 已过黑盒；下一步 **P7 全面整理与优化**）  
...
**验收（M7）**：调向时可透视观察带面；中枢职责可说明「谁管逻辑、谁管视觉」；无双轨静态工具；玩家文案与调试收口达标；回归 P0–P6 黑盒不回退。具体拆分方案与文件表由执行者写入本计划「现状/落点」节或变更说明，**不**在开工前臆造类名。

---

## 7. 建议文件落点（**现状索引**；P7 拆分后由执行者更新本表）

| 路径 | 职责 | 现况 |
| --- | --- | --- |
| `Subsystems/SubsystemBeltGroups.cs` | Group 发现 / 存档 / 脏重建 / 运物仿真；F2 调试（P7 待拆） | ✅ P0–P6；过重 |
...
| `Belt/BeltSegmentInventory.cs` | IInventory 窗口语义；**P7** 静态收敛时处理 | ✅ P2（待收敛） |
| `Components/ComponentConveyerBeltSegment.cs` | 实体上挂门面 | ✅ P2 |
| `Blocks/ConveyerBeltBlock.cs` | 几何 / Data reverse·powered / 交互 | ✅ P6 |
| `Subsystems/SubsystemConveyerBeltBlockBehavior.cs` | 朝向/爬坡；Rebuild；调向入口 | ✅ P6 |
| `Subsystems/SubsystemConveyerBeltVisuals.cs` | 全局 RT：Update 合成 | ✅ P6（P7 视/逻辑边界再理） |
...
| 中枢上帝类 / 视逻辑同子系统 | **P7**：拆分下沉 + View/Model 分离（方案由执行者定） |
...
下一步 **P7 全面整理与优化**（透视调向 UI、中枢拆分、View/Model 分离、静态收敛、文案/调试/按需性能）。执行前先出具体方案再改代码。

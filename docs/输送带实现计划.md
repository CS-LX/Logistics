...
- 以后可选：离散档（停 / 慢 / 快）、每 Group 独立 RT、或着色器时间 UV（**P7**）
...
**不在本期**：离散多档速、每 Group 独立 RT、图鉴全文、Extensions 收敛。

### P7 — 工程与可选打磨

1. 离散速度档 + 材质档（若仍用单 RT，则仅改全局或接受「档位=全局」）  
2. 最大 Group 长度、性能剖析（含：极低视距超长带进档卡顿）  
3. 图鉴 / Lang 更新（玩家文案，禁止路径 / 类名泄露）  
4. **静态辅助收敛**（见下）  
5. （可选）进档 / 区块边界在途物散落：Rebuild 迁回与坡道姿态在半加载时的稳健性（见 §8）  
6. 收掉 F2 / 点击调试残留（或收进开发者开关）  

#### P7.5 — `ConveyerBeltExtensions` / `ConveyerBeltUtils`

P0–P6 期间允许 `BeltPath`、`BeltSegmentInventory` 等静态类与散落 helper 存在。P7 统一收拢：
...
| `Belt/BeltSegmentInventory.cs` | IInventory 窗口语义（P2）；P7.5 可迁入 Extensions/Utils | ✅ P2（待收敛） |
| `Components/ComponentConveyerBeltSegment.cs` | 实体上挂门面 | ✅ P2 |
| `Belt/ConveyerBeltExtensions.cs` | 本模块类型扩展方法 | 待建（P7.5） |
| `Belt/ConveyerBeltUtils.cs` | 非扩展静态工具 | 待建（P7.5） |
| `Blocks/ConveyerBeltBlock.cs` | 几何 / 渲染 | ✅ 已有 |
| `Subsystems/SubsystemConveyerBeltBlockBehavior.cs` | 朝向/爬坡（保 reverse/powered）；Rebuild；调向入口 | ✅ P6 |
| `Subsystems/SubsystemConveyerBeltVisuals.cs` | 全局 RT：Update 合成；有运转则滚 | ✅ P6 |
| `Drawing/ConveyerBeltAnimatedTexture.cs` | 固定极性滚动 RT | ✅ P6 |
| `Blocks/ConveyerBeltBlock.cs` | Data reverse/powered；32 mesh；IsInteractive | ✅ P6 |
...
| 长 Group 单点 tick / 进档卡顿 | 限制最大长度或分帧；**P7** 再测 |
| 进档压力测试在途物散落 Pickable | **延后 P7（可选）**：视距 32 反复进退可复现；非体验闭环 blocker |
...
| M7 | 图鉴/Lang；Extensions/Utils；可选性能与低视距散落 |

---

## 11. 下一步

下一步 **P7 工程打磨**（Extensions/Utils、图鉴、性能、调试残留等）。

*计划修订：2026-07-25 — P6 已过黑盒（SA 式 reverse/powered + 调向交互）。*

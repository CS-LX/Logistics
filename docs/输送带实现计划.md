...
> 状态：**施工中**（P0–P2 已过黑盒；下一步 P2.1 进世界稳健，再 P3）  
...
**验收**：进世界、跨区块加载、存档 Save 路径不再出现上述 NRE。
...
#### P5.5 — `ConveyerBeltExtensions` / `ConveyerBeltUtils`

P0–P4 期间允许 `BeltPath`、`BeltSegmentInventory` 等静态类与散落 helper 存在。打磨阶段统一收拢：
...
| `Belt/BeltSegmentInventory.cs` | IInventory 窗口语义（P2）；P5.5 可迁入 Extensions/Utils | ✅ P2（待收敛） |
| `Components/ComponentConveyerBeltSegment.cs` | 实体上挂门面 | ✅ P2 |
| `Belt/ConveyerBeltExtensions.cs` | 本模块类型扩展方法 | 待建（P5.5） |
| `Belt/ConveyerBeltUtils.cs` | 非扩展静态工具 | 待建（P5.5） |
...
| 区块未加载成员 / 进世界偶发 NRE | 见 **P2.1**：`IsBeltCell` 等禁止对未加载 chunk 调 `GetCellValueFastChunkExists`（宿主实现假定 chunk 非 null）；未加载格视为「非带」或延后 Rebuild |
...
**已知缺陷（2026-07-25）**：进世界偶发 `NullReferenceException` @ `SubsystemBeltGroups.IsBeltCell` → `Terrain.GetCellValueFastChunkExists`（例：邻格/成员落在未加载区块，如 `(-128,69,59)`）。**排期：P2.1（P3 之前）**，勿拖到 P5。
...
| M3 | 直角两带速度继承手感可接受 |
| M4 | 站立实体被推动；潜行可停 |
| 打磨 | Sign 可调；文案与图鉴；无调试残留；静态辅助 → Extensions/Utils |

---

## 11. 下一步

下一步 **P2.1**（进世界未加载 chunk NRE），再进 **P3** 直角速度继承。

*计划修订：2026-07-25 — P0–P2 黑盒通过；P2.1 进世界稳健优先于 P3；P5.5 Extensions/Utils 收敛。*

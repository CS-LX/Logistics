...
> 状态：**施工中**（P0–P2.1 已过黑盒；下一步 P3 → P4 机械能 → P5 推实体）  
...
- **机械能停/转**（有动力才推进）：首发硬需求，排期 **P4**（接 `SubsystemEnginePower`）；与「无极幅度」无关
...
1. Group 判定 `IsRunning`：**任一段** Members 的**两侧**（垂直于带走向的左右邻格，不含沿带前后端）邻接有机械能即可驱动**整组**  
2. `TickInventories`：仅 `IsRunning` 时推进；停转时在途物保留、臂仍可抽塞  
3. 滚动贴图：全局 RT 与仿真脱节已知；P4 至少保证**物品不动**；视觉停滚若可行再做，否则记 P6  
4. 黑盒：撤掉 / 熄火蒸汽机等使邻接无机械能 → 停；再接上并运转 → 恢复；与抓取机 / 末端弹出不冲突  

**验收**：失去机械能后物品停在带上；重新获得机械能后继续滑；不因停转把在途物清成 Pickable。中段任一侧接引擎即可驱动整条直线 Group。
...
2. **仅 `IsRunning` 时推动**；与物品仿真分离；坡道用 `rotation` / `shape` 轨迹  
3. 潜行可停  

**验收**：运转中玩家 / 生物站上被带走；停转或潜行不推。

### P6 — 打磨（可选，非首发 blocker）

1. Controller / 任意段交互：切换 `Sign`（恢复轻量 UI 亦可）  
2. 离散速度档 + 材质档（若仍用单 RT，则仅改全局或接受「档位=全局」）  
3. 最大 Group 长度、性能剖析（含：极低视距超长带进档卡顿）  
4. 图鉴 / Lang 更新（玩家文案，禁止路径 / 类名泄露）  
5. **静态辅助收敛**（见下；玩法稳定后再动，非 M0–M4 blocker）  
6. （可选）进档 / 区块边界在途物散落：Rebuild 迁回与坡道姿态在半加载时的稳健性（见 §8）  
7. （可选）停转时全局滚动贴图观感对齐  

#### P6.5 — `ConveyerBeltExtensions` / `ConveyerBeltUtils`

P0–P5 期间允许 `BeltPath`、`BeltSegmentInventory` 等静态类与散落 helper 存在。打磨阶段统一收拢：

| 落点 | 放什么 |
| --- | --- |
| `Belt/ConveyerBeltExtensions.cs`（或 `Extensions/`） | **扩展方法**：仅当「升格门禁」满足 |
| `Belt/ConveyerBeltUtils.cs` | 其余静态工具（无合适 `this` 接收者、或跨宿主类型的纯函数） |

**扩展方法升格门禁**：第一个参数必须是**本模块专有**类 / 结构（如 `BeltGroup`、`BeltInventory`、`TransportedItem`、`ConveyerBeltBlock` 相关本仓类型）。  
【禁止】仅为调用顺口，给 `Point3` / `Vector3` / 宿主 `Entity` / 原版 `Subsystem*` 等非本模块类型挂扩展。

收敛时：删掉已迁入的旧静态类空壳，避免双轨；行为与黑盒不变。
...
| 路径 | 职责 | 现况 |
| --- | --- | --- |
| `Subsystems/SubsystemBeltGroups.cs` | Group 发现 / 存档 / 脏重建 / 运物仿真；F2 调试 | ✅ P0+P1 |
...
| `Belt/BeltSegmentInventory.cs` | IInventory 窗口语义（P2）；P6.5 可迁入 Extensions/Utils | ✅ P2（待收敛） |
| `Components/ComponentConveyerBeltSegment.cs` | 实体上挂门面 | ✅ P2 |
| `Belt/ConveyerBeltExtensions.cs` | 本模块类型扩展方法 | 待建（P6.5） |
| `Belt/ConveyerBeltUtils.cs` | 非扩展静态工具 | 待建（P6.5） |
| `Blocks/ConveyerBeltBlock.cs` | 几何 / 渲染 | ✅ 已有 |
| `Subsystems/SubsystemConveyerBeltBlockBehavior.cs` | 朝向/爬坡；Rebuild；吸抛射物；Segment 实体 | ✅ P0–P2 |
| `Subsystems/SubsystemConveyerBeltVisuals.cs` | 滚动贴图驱动 | ✅ 已有 |
| `Drawing/ConveyerBeltAnimatedTexture.cs` | 滚动 RT | ✅ 已有；变速视觉后置 |
| `Logistics.xdb` | Subsystem + EntityTemplate | ✅ BeltGroups + ConveyerBeltSegment |
...
| 风险 | 缓解 |
| --- | --- |
| 长 Group 单点 tick / 进档卡顿 | 限制最大长度或分帧；P5 再测。极低视距（32）超长带进档 ~0.25–0.5s 可接受延后 |
| 区块未加载 / 进世界 NRE | **P2.1 已过**：`TryGetCellValue`；未加载不拆残缺组；`GetCellValueFast` |
| 进档压力测试在途物散落 Pickable | **延后 P5（可选）**：视距 32 反复进退可复现，坡/端口 > 平直；视距 512 基本无。非首发 blocker（极端视距 + 玩家已知卸载停摆） |
...
| 打磨 | Sign 可调；文案与图鉴；无调试残留；静态辅助 → Extensions/Utils；可选：低视距进档/散落、停转贴图 |

---

## 11. 下一步

下一步 **P3** 直角速度继承 → **P4** 机械能驱动 → **P5** 实体推动。

*计划修订：2026-07-25 — P0–P2.1 已过；机械能停/转升格为 **P4**；原实体推动顺延 P5、打磨 P6。*
